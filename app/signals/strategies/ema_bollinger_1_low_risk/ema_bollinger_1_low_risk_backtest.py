...
class MyStrat(Strategy):
    ...
    def next(self):
        ...
        if self.signal1 == 2 and len(self.trades) < _strategy_parameters.get(
            "max_longs", 1
        ):
        ...
        elif self.signal1 == 1 and len(self.trades) < _strategy_parameters.get(
            "max_shorts", 1
        ):
            sl1 = self.data.Close[-1] + slatr
            tp1 = self.data.Close[-1] - slatr * TPSLRatio
...
def backtest(df, strategy_parameters, size=0.03, skip_optimization=False, best_params=None):
    ...
    MyStrat.mysize = lot_size
    MyStrat.initsize = lot_size
    MyStrat.slcoef = strategy_parameters.get("slcoef", 2.3)
    MyStrat.TPSLRatio = strategy_parameters.get("tpslRatio", 2.5)
    MyStrat.trades_actions = []
    ...
    bt_best = Backtest(dftest, MyStrat, cash=cash, margin=margin, finalize_trades=True)
    stats = bt_best.run()
