...
class EnhancedStrategy(Strategy):
    ...
    slcoef = 3.0           # ATR multiplier for stop loss - increased from 2.0
    TPSLRatio = 2.5     # Take profit to stop loss ratio - increased from 2.0
    trailing_sl = 2.0  # Trailing stop ATR multiplier - increased from 1.5
    max_positions = 1  # Maximum concurrent positions
    max_risk_pct = 1.0  # Maximum risk per trade (%)
    min_atr_value = 0.0001  # Minimum ATR value to prevent tiny stops
    trades_actions = []
    mysize = 0.01  # Match variable name from MyStrat for compatibility
    ...
    def next(self):
        ...
        for trade in list(self.trades):
            ...
            if trade.is_long:
                ...
                if self.data.Low[-1] < self.trailing_stops[trade_id]:
                    trade.close()
                    if _strategy_parameters.get('best', False):
        ...
        if self.signal == 2 and len([t for t in self.trades if t.is_long]) == 0:
            ...
            if dollar_risk_per_unit > 0:
                ...
                pos_size = min(pos_size, _base_lot_size * 2.0)  # Cap position size
                pos_size = max(pos_size, _base_lot_size * 0.5)  # Minimum position size
            else:
                pos_size = _base_lot_size
...
def backtest(df, strategy_parameters, size = 0.01, skip_optimization=False, best_params=None):
    global _dftest, _strategy_parameters, _base_lot_size
    ...
    EnhancedStrategy.slcoef = strategy_parameters.get("slcoef", 3.0)
    EnhancedStrategy.TPSLRatio = strategy_parameters.get("tpslRatio", 2.5)
    EnhancedStrategy.trailing_sl = strategy_parameters.get("trailing_sl", 2.0)
    EnhancedStrategy.max_positions = strategy_parameters.get("max_positions", 1)
    EnhancedStrategy.max_risk_pct = strategy_parameters.get("max_risk_pct", 1.0)
    EnhancedStrategy.min_atr_value = strategy_parameters.get("min_atr_value", 0.0001)
    EnhancedStrategy.mysize = base_lot_size
    EnhancedStrategy.trades_actions = []
    ...
    EnhancedStrategy.slcoef = best_params.get('slcoef', 4.0)
    EnhancedStrategy.TPSLRatio = best_params.get('TPSLRatio', 3.0)
    EnhancedStrategy.trailing_sl = best_params.get('trailing_sl', 2.5)
    EnhancedStrategy.max_positions = best_params.get('max_positions', 1)
    EnhancedStrategy.max_risk_pct = best_params.get('max_risk_pct', 1.0)
    EnhancedStrategy.min_atr_value = best_params.get('min_atr_value', 0.0005)
